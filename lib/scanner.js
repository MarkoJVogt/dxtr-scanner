...
import {
  SECRET_PATTERNS,
  PII_LOG_PATTERNS,
  AI_ACT_HIGH_RISK_PATTERNS,
  AI_ACT_GENERATIVE_PATTERNS,
  AI_ACT_RELEVANT_LIBS,
  SCAN_EXTENSIONS,
  IGNORE_DIRS,
  MAX_FILE_SIZE_BYTES,
  MAX_FILES_SCANNED,
...
function scanContent(content) {
  const secretHits = [];
  const piiHits = [];
  const aiActHighRiskHits = [];
  const aiActGenerativeHits = [];
  ...
  for (const { name, regex } of AI_ACT_HIGH_RISK_PATTERNS) {
    regex.lastIndex = 0;
    if (regex.test(content)) aiActHighRiskHits.push(name);
  }
  for (const { name, regex } of AI_ACT_GENERATIVE_PATTERNS) {
    regex.lastIndex = 0;
    if (regex.test(content)) aiActGenerativeHits.push(name);
  }
  return { secretHits, piiHits, aiActHighRiskHits, aiActGenerativeHits };
}

function matchesAiActLib(dependencyName) {
  const name = dependencyName.toLowerCase();
  for (const candidate of AI_ACT_RELEVANT_LIBS) {
...
function evaluate({ secretHits, piiHits, playbookMatches, envProtection, aiActHighRiskHits, aiActGenerativeHits, aiLibsDetected }) {
  const sections = [];
  ...
  if (aiActHighRiskHits.length > 0) {
    const names = [...new Set(aiActHighRiskHits.map((h) => h.name))];
    sections.push({
      id: "ai_act",
      title: "Mögliche EU-AI-Act-Hochrisiko-Indikatoren gefunden",
      level: "red",
      reason: `${aiActHighRiskHits.length} Stelle(n) mit Hinweisen auf Anhang-III-Anwendungsfälle der KI-VO gefunden (${names.join(", ")}). Solche Muster (z.B. Biometrie, Emotionserkennung, Bonitäts-/Recruiting-Scoring, Social Scoring, Strafverfolgung, automatisierte Entscheidungen) können eine Hochrisiko-Einstufung nach Art. 6 i.V.m. Anhang III der Verordnung (EU) 2024/1689 auslösen.`,
      legalQuestion:
        "Ist der Anwendungsfall formal als Hochrisiko-KI-System nach Anhang III eingestuft worden, und sind die zugehörigen Pflichten (Risikomanagement, Datenqualität, technische Dokumentation, menschliche Aufsicht, Konformitätsbewertung) umgesetzt?",
    });
  } else if (aiLibsDetected.length > 0 || aiActGenerativeHits.length > 0) {
    const names = [...new Set([...aiLibsDetected, ...aiActGenerativeHits.map((h) => h.name)])];
    sections.push({
      id: "ai_act",
      title: "KI-/LLM-Komponenten erkannt — Einstufung nach EU AI Act zu prüfen",
      level: "yellow",
      reason: `Im Tech-Stack bzw. Code wurden KI-/LLM-Komponenten identifiziert (${names.join(", ")}). Es wurden keine erkennbaren Hochrisiko-Indikatoren gefunden, dennoch löst der Einsatz von KI-Systemen regelmäßig Prüf- und Dokumentationspflichten nach der KI-VO (Verordnung (EU) 2024/1689) aus.`,
      legalQuestion:
        "Wurde der Anwendungsfall formal nach Art. 6/Anhang III KI-VO als Nicht-Hochrisiko eingestuft (mit Begründung), und sind bei Interaktion mit Nutzern die Transparenzpflichten nach Art. 50 KI-VO (z.B. Kennzeichnung als KI-generiert/Chatbot) erfüllt?",
    });
  } else {
    sections.push({
      id: "ai_act",
      title: "Keine EU-AI-Act-relevanten KI-Komponenten erkannt",
      level: "green",
      reason: "Im Rahmen der geprüften Muster und Dependencies wurden keine KI-/LLM-Komponenten oder Hochrisiko-Indikatoren nach der KI-VO gefunden.",
      legalQuestion: null,
    });
  }
  ...
  try {
    ...
    for (const file of files) {
      ...
      const {
        secretHits: sHits,
        piiHits: pHits,
        aiActHighRiskHits: aHits,
        aiActGenerativeHits: gHits,
      } = scanContent(content);
      ...
      for (const name of aHits) aiActHighRiskHits.push({ name, file: file.relativePath });
      for (const name of gHits) aiActGenerativeHits.push({ name, file: file.relativePath });
    }
    ...
    const aiLibsDetected = [...new Set(dependencies.filter(matchesAiActLib))];
    ...
    return {
      ...
      findings: {
        ...
        aiActFindings: { highRiskHits: aiActHighRiskHits, generativeHits: aiActGenerativeHits, aiLibsDetected },
        envProtection,
      },
      evaluation,
    };
